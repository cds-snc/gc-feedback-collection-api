...
1. **Input**:
   - Web form → API Gateway → `queue_problem_form` Lambda
   - Email → SES → SNS → `queue_problem` Lambda
...
1. **Input**:
   - Web form → API Gateway → `queue_toptask_survey_form` Lambda
   - Email → SES → SNS → `queue_toptask` Lambda
